...
public class ChessGameGUI extends JFrame implements MouseListener, MouseMotionListener, Observer {
	...
	private void emptyGrid() {
        ...
        for(int i = 0; i < 8; i++) {
            for(int j = 0; j < 8; j++) {
                panel = (JPanel) chessBoardGuiContainer.getComponent(i+j*8);
                panel.removeAll();
            }
        }
    }
    ...
    public void update(Observable o, Object arg) {
	    ...
	    this.emptyGrid();
	    this.setPieces((List<PieceIHM>) arg);
    }


    private void setPieces(List<PieceIHM> piecesIHM) {
        /* Recuperation des donnees des pieces et ajout sur le plateau */
        for(PieceIHM p : piecesIHM) {
            String name = p.getNamePiece();
            Couleur couleur = p.getCouleur();
            int valMap = p.getX() + p.getY() * 8;
            JLabel piece;
            JPanel panel;
            String path = ChessImageProvider.getImageFile(name, couleur);
            piece = new JLabel(new ImageIcon(path));
            panel = (JPanel) chessBoardGuiContainer.getComponent(valMap);
            panel.add(piece);
        }
    }
}
